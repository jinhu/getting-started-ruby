...
RSpec.describe GeekItemsController, type: :controller do
  before do
    # allow(request.env['warden']).to receive(:authenticate!) { user }
    user = double('user')
    allow(user).to receive(:id) { "123" }
    allow(controller).to receive(:current_user) { user }
    allow(user).to receive(:has_permission_for) { true }

  end

  scenario "index page for geek items" do
    get :index
    expect(response).to have_http_status(:success)
  end

  scenario "new geek item page for geek items" do
    get :new
    expect(response).to have_http_status(:success)
  end
  scenario "create geek item page for geek items" do
    post :create, geek_item:{ item_id: "123", status: "todo"}
    expect(response).to have_http_status(302)
  end
  scenario "show existing geek items result in not found" do
    item = GeekItem.create user_id: "123", item_id: "123", status: "todo"
    get :show, id: item.id
    expect(response).to have_http_status(:success)
  end

  # scenario "Listing all books" do
  #   GeekItem.create! user_id: "Super mario bros", item_id: "Shigeru Miyamoto"
  #
  #   visit root_path
  #
  #   expect(page).to have_content "Super mario bros"
  #   expect(page).to have_content "Shigeru Miyamoto"
  # end
  #
  ...
  # end
  #
  # scenario "Adding a Geek item" do
  #   visit root_path
  #   click_link "Add Geek item"
